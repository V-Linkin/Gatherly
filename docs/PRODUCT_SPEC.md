...
用户打开 App
    ↓
首页显示多行粘贴框（支持含分享文案的混合文本，自动识别 URL）
    ↓
用户粘贴链接（自动提取 URL）→ 点击「一键归档」
    ↓
...
### 问题解决
- ✅ **正文内容获取**: 通过镜像站完整提取
- ✅ **图片列表获取**: 过滤非内容图片，保留真实文章图片
- ✅ **作者信息获取**: 从 feed 数据提取用户名

### 技术实现
- **三级降级架构**: 镜像站 (coolapk1s.com) → 原站 HTTP → WKWebView
- **镜像站解析**: 从 `__NEXT_DATA__` JSON 提取 feed 数据（标题/正文/作者/图片）
- **图片代理**: 通过 `image.coolapk1s.com/proxy?url=` 绕过防盗链
- **正文清理**: 移除 HTML 标签（`<a class="feed-link-tag">`）和表情标签（`[酷币]`）
- **首图去重**: 封面与首图相同时从正文图片列表中移除

### 已知问题更新
1. **酷安网页版反爬严格** ✅ 已解决
   - 使用 coolapk1s.com 镜像站绕过反爬
   - 镜像站基于 Next.js SSR，无反爬限制
