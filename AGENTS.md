...
- **抖音解析**: `DouyinParser` 使用移动端 User-Agent 获取页面，从 `window._ROUTER_DATA` 提取数据（路径：`loaderData.note_(id)/page` 或 `video_(id)/page`）。通过 `aweme_type` 区分内容类型（0=视频, 2=图文）。图文笔记提取 `images[].url_list[0]` 作为图片列表，封面与首图按 URL 去重；视频笔记提取 `video.play_addr.url_list`。标题优先使用 `title` 字段，无则从 `desc` 去掉 `#话题` 后截取前50字。
